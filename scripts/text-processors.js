...
function processTextPotencyReferences(text, monster) {
  ...
  // Handle @potency.* that's already inside HTML - replace with proper characteristic<X formatting
...
function flattenPowerEffects(item, monster) {
  ...
  // Otherwise, build tiers from effects structure
  ...
  for (let tierNum = 1; tierNum <= 3; tierNum++) {
    ...
    Object.values(effects).forEach(effect => {
      ...
      if (effect.type === 'applied' && effect.applied?.[tierKey]) {
        ...
        // Generate potency display
        if (applied.potency?.value !== undefined) {
          ...
          // Generate effect names and duration text
          const effectNames = Object.keys(applied.effects || {});
          if (effectNames.length > 0) {
            appliedText = `${charAbbrev}<${applied.potency.value} ${effectNames.join(', ')}`;
      ...
      if (effect.type === 'forced' && effect.forced?.[tierKey]) {
        ...
        if (forced.display) {
          ...
          const cleanDisplay = forced.display.replace(/<[^>]*>/g, '').trim();
          if (cleanDisplay) {
